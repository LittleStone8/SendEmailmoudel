...
System.out.println("+++++++++"+parameters.facility.ownerPartyId);
partyGroup = delegator.findByPrimaryKey("PartyGroup", [partyId : parameters.facility.ownerPartyId]);
if (partyGroup?.logoImageUrl) {
    logoImageUrl = partyGroup.logoImageUrl;
   
}

OrderItemShipGroupinfo = delegator.findByPrimaryKey("OrderItemShipGroup", [orderId : orderId,shipGroupSeqId:"00001"]);

context.logoImageUrl=logoImageUrl;
productStore = delegator.findByPrimaryKey("ProductStore", [productStoreId : orderHeader.productStoreId]);
if (productStore?.storeName) {
    stroename = productStore.storeName;
}
context.stroename=stroename;

address = delegator.findByPrimaryKey("PostalAddress", [contactMechId : OrderItemShipGroupinfo.contactMechId]);
context.postalAddress = address;

context.facilityId=parameters.facility.facilityId;

context.orderId=orderId;

context.facilityName=parameters.facility.facilityName;

context.facilityId=parameters.facility.facilityId;

context.carrierPartyId=OrderItemShipGroupinfo.carrierPartyId;

context.shipmentMethodTypeId=OrderItemShipGroupinfo.shipmentMethodTypeId;
	
context.shipGroups=OrderItemShipGroupinfo;


context.put("orderId222", orderId);
/*
OrderItemShipGroupinfo = delegator.findByPrimaryKey("OrderItemShipGroup", [orderId : orderId,shipGroupSeqId:"00001"]);
...
// the address
partyId=orh.getBillToParty().partyId;
//System.out.println(orh.getBillToParty().partyId);
...
context.phone = delegator.findByPrimaryKey("TelecomNumber", [contactMechId : OrderItemShipGroupinfo.telecomContactMechId]);
// Fax
