...
//        }
                ...
                if (availableQuantity.compareTo(thisCancelQty) >= 0) {
                    ...
                    if (orderItemQtyCancelled.compareTo(originalItemQuantity) >= 0) {
                        ...
                        try {
        					...
        					exprs.add(EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS, orderItemShipGroupAssoc.getString("orderItemSeqId")));

                        	List<GenericValue> orderItemInventorys = delegator.findByCondition("OrderItemShipGrpInvRes", EntityCondition.makeCondition(exprs), null, null);
                        	GenericValue orderItemInventory = orderItemInventorys.get(0);
                        	
                        	List<EntityExpr> exprs2 = FastList.newInstance();
                        	exprs2.add(EntityCondition.makeCondition("orderItemSeqId", EntityOperator.EQUALS, orderItemShipGroupAssoc.getString("orderItemSeqId")));
        					List<GenericValue> orderItemss = delegator.findByCondition("OrderItem", EntityCondition.makeCondition(exprs2), null, null);
                        	GenericValue orderItems = orderItemss.get(0);
                        	GenericValue orderHeader = delegator.findByPrimaryKey("OrderHeader",UtilMisc.toMap("orderId", orderId));
                        	String oldStatusId = orderHeader.getString("statusId");
                        	GenericValue inventoryItem = delegator.findByPrimaryKey("InventoryItem",UtilMisc.toMap("inventoryItemId", orderItemInventory.getString("inventoryItemId")));
                        	Map statusCtx = null;
                        	BigDecimal qoh=null;
                        	BigDecimal atp=null;
                        	if("ORDER_COMPLETED".equals(oldStatusId)||"ORDER_COMPLETED_W_P".equals(oldStatusId)){
                        		statusCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItem.getString("orderItemSeqId"), "statusId", "ITEM_CANCELLED", "userLogin", userLogin,"operator",userLogin.get("userLoginId")+"("+userLogin.getString("partyId")+")","usageType",UsageTypeEnum.OrderCanceled.getName(),"qoh",inventoryItem.getBigDecimal("quantityOnHandTotal").add(orderItems.getBigDecimal("quantity")),"atp",inventoryItem.getBigDecimal("availableToPromiseTotal").add(orderItems.getBigDecimal("quantity")),"oldStatusId",oldStatusId);
                        		qoh=inventoryItem.getBigDecimal("quantityOnHandTotal").add(orderItems.getBigDecimal("quantity"));
                        		atp=inventoryItem.getBigDecimal("availableToPromiseTotal").add(orderItems.getBigDecimal("quantity"));
                        	}else{
                            	statusCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItem.getString("orderItemSeqId"), "statusId", "ITEM_CANCELLED", "userLogin", userLogin,"operator",userLogin.get("userLoginId")+"("+userLogin.getString("partyId")+")","usageType",UsageTypeEnum.OrderCanceled.getName(),"qoh",inventoryItem.getBigDecimal("quantityOnHandTotal"),"atp",inventoryItem.getBigDecimal("availableToPromiseTotal").add(orderItems.getBigDecimal("quantity")));
                            	qoh = inventoryItem.getBigDecimal("quantityOnHandTotal");
                            	atp=inventoryItem.getBigDecimal("availableToPromiseTotal").add(orderItems.getBigDecimal("quantity"));
                            }
                        	
                        	//setItemStatus(ctx,statusCtx);
                        	dispatcher.runSyncIgnore("changeOrderItemStatus", statusCtx);
                        	
                        	updateInventoryItemById(delegator, inventoryItem.getString("inventoryItemId"), null, null, null, null, null, qoh, atp, null, null, null, null, null, null);
                        	String inventoryItemTypeId=inventoryItem.getString("inventoryItemTypeId");
                        	if(!"NON_SERIAL_INV_ITEM".equals(inventoryItemTypeId))
                        		insertInventoryItemDetail(delegator, new Timestamp(System.currentTimeMillis()), inventoryItem.getString("inventoryItemId"), orderItems.getBigDecimal("quantity"), orderItems.getBigDecimal("quantity"), qoh, atp, UsageTypeEnum.OrderCanceled.getName(), userLogin.get("userLoginId")+"("+userLogin.getString("partyId")+")", null, null, null, null, null,orderId);
                        ...
                        try {
                        	GenericValue orderItemInventory = delegator.findByPrimaryKey("OrderItemShipGrpInvRes",UtilMisc.toMap("orderItemSeqId", orderItemShipGroupAssoc.getString("orderItemSeqId")));
                        	
                        	GenericValue orderItems = delegator.findByPrimaryKey("OrderItem",UtilMisc.toMap("orderItemSeqId", orderItemShipGroupAssoc.getString("orderItemSeqId")));
                        	GenericValue inventoryItem = delegator.findByPrimaryKey("InventoryItem",UtilMisc.toMap("inventoryItem", orderItemInventory.getString("inventoryItemId")));
                        	  Map invCtx = UtilMisc.toMap("orderId", orderId, "orderItemSeqId", orderItem.getString("orderItemSeqId"), "shipGroupSeqId",
                                      shipGroupSeqId, "cancelQuantity", thisCancelQty, "userLogin", userLogin,"operator",userLogin.get("userLoginId")+"("+userLogin.getString("partyId")+")","usageType",UsageTypeEnum.OrderCanceled.getName(),"qoh",inventoryItem.getBigDecimal("quantityOnHandTotal"),"atp",inventoryItem.getBigDecimal("availableToPromiseTotal").add(orderItems.getBigDecimal("quantity")));
    ...
    public static Map setOrderStatus(DispatchContext ctx, Map context) {
        ...
        Security security = ctx.getSecurity();
        boolean hasPermission = OrderServices.hasPermission(orderId, userLogin, "UPDATE", security, delegator);
        boolean CANCEL= security.hasEntityPermission("BIM_ORDER_","CANCEL",userLogin);
        boolean COMPLETE=security.hasEntityPermission("BIM_ORDER_","COMPLETE",userLogin);

    
        if (!hasPermission&&!CANCEL&&!COMPLETE) {
            return ServiceUtil.returnError(UtilProperties.getMessage(resource_error,"OrderYouDoNotHavePermissionToChangeThisOrdersStatus",locale));
        }
	...
	public static void insertInventoryItemDetail(Delegator delegator,Timestamp effectiveDate,String inventoryItemId,BigDecimal availableToPromiseDiff
			...
			,String usageType,String operator,String facilityName,String facilityToName,String transferId,String reasonEnumId,String statusId,String orderId) throws GenericEntityException{
		...
		newInventoryItemDetail.set("orderId", orderId);
