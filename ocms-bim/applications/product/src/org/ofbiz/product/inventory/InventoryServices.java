...
public class InventoryServices {
    ...
    public static Map<String, Object> completeInventoryTransfer(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            String fromFacilityId = inventoryTransfer.getString("facilityId");
            String toFacilityId = inventoryTransfer.getString("facilityIdTo");
            
            facility = delegator.findByPrimaryKey("Facility",
                    UtilMisc.toMap("facilityId", fromFacilityId));
            fromFacilityName = facility.getString("facilityName");
            
            facility = delegator.findByPrimaryKey("Facility",
                    UtilMisc.toMap("facilityId", toFacilityId));
            toFacilityName = facility.getString("facilityName");
        ...
        if (inventoryType.equals("NON_SERIAL_INV_ITEM")) {
            ...
            Map<String, Object> createDetailMap = UtilMisc.toMap("availableToPromiseDiff", qoh.subtract(atp),
                    "inventoryItemId", inventoryItem.get("inventoryItemId"), "userLogin", userLogin,"operator",partyId,"usageType",UsageTypeEnum.TransferCompleted.getName(),"qoh",qoh,"atp",qoh,"fromWarehouse",fromFacilityName,"toWarehouse",toFacilityName);
            try {
                Map<String, Object> result = dctx.getDispatcher().runSync("createInventoryItemDetail", createDetailMap);
                if (ServiceUtil.isError(result)) {
                    return ServiceUtil.returnError("Inventory Item Detail create problem in complete inventory transfer", null, null, result);
    ...
    public static Map<String, Object> getProductInventoryAndFacilitySummary(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String sql = "SELECT b.product_id, GROUP_CONCAT(DISTINCT a.DESCRIPTION ORDER BY a.PRODUCT_FEATURE_TYPE_ID) as description,"
        		+ " c.BRAND_NAME as brandName,c.INTERNAL_NAME as model FROM PRODUCT_FEATURE a"
        ...
        try{
        	...
        	while(rs.next()){
        		...
        		result.put("description", str.toString());
        	}
