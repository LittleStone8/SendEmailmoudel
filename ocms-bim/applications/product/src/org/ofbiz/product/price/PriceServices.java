...
//        }
                    ...
                    Map<String, Object> calcResults =  priceRule(listPrice, defaultPrice, promoPrice, wholesalePrice, maximumPriceValue, minimumPriceValue, validPriceFound, averageCostValue, productId, virtualProductId, prodCatalogId, productStoreGroupId, webSiteId, partyId, quantity, currencyUomId, delegator, nowTimestamp, productStoreId);
            	    result.putAll(calcResults);
    ...
    public static Map<String, Object> priceRule(BigDecimal listPrice, BigDecimal defaultPrice, BigDecimal promoPrice,
	        ...
	        String webSiteId, String partyId, BigDecimal quantity, String currencyUomId, Delegator delegator, Timestamp nowTimestamp,String productStoreId)  {
	
    
	BigDecimal changePrice = null;
	
	
//      //价格规则 暂时 只有一店一价 所以只有一个条件
      GenericHelperInfo helperInfo = delegator.getGroupHelperInfo("org.ofbiz");
	SQLProcessor processor = new SQLProcessor(helperInfo);
	StringBuffer onePriceRuleSql = new StringBuffer("select ppa.AMOUNT as AMOUNT from PRODUCT_PRICE_ACTION ppa "
		+ "where ppa.PRODUCT_PRICE_RULE_ID = (select ppr.PRODUCT_PRICE_RULE_ID from PRODUCT_PRICE_RULE ppr where "
		+ "ppr.PRODUCT_PRICE_RULE_ID in (SELECT ppc1.PRODUCT_PRICE_RULE_ID FROM PRODUCT_PRICE_COND ppc1 "
		+ "join PRODUCT_PRICE_COND ppc2 on (ppc1.PRODUCT_PRICE_RULE_ID = ppc2.PRODUCT_PRICE_RULE_ID) "
		+ "where  ppc1.`INPUT_PARAM_ENUM_ID` = 'PRIP_PRODUCT_ID' and ppc1.`OPERATOR_ENUM_ID`='PRC_EQ' and ppc1.`COND_VALUE`='"+productId+"' and "
		+ "ppc2.`INPUT_PARAM_ENUM_ID` = 'PRIP_PRODUCT_STORE_ID' and ppc2.`OPERATOR_ENUM_ID`='PRC_EQ' and ppc2.`COND_VALUE`='"+productStoreId+"' ) "
		+ "and ppr.FROM_DATE <= CURDATE() and ( ppr.THRU_DATE >= CURDATE() or ppr.THRU_DATE is null ) order by ppr.CREATED_TX_STAMP desc limit 1)"
		);

	System.out.println(onePriceRuleSql.toString());
		
	try {
	    ResultSet rs = processor.executeQuery(onePriceRuleSql.toString());
	    while (rs.next()) {
	       changePrice =   rs.getBigDecimal("AMOUNT");
	    }
	    rs.close();
	    processor.close();
	...
	if("RETAIL".equals(productStoreGroupId)){
	    defaultPrice = changePrice == null ? defaultPrice :changePrice;
	}
