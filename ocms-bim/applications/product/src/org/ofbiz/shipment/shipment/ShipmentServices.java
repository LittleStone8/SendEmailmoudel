...
import org.ofbiz.shipment.bean.Drverinfobean;
...
public class ShipmentServices {
	...
	public static Drverinfobean queryEGATEEdriver(Delegator delegator, String party) throws GenericEntityException {
		Map<String, Object> results = ServiceUtil.returnSuccess();
		
		if(party==null||"".equals(party))
		{
			Drverinfobean temp = new Drverinfobean("", "", "");
			return temp;
		}
		
		
		String sql="select p.PARTY_ID,p.FIRST_NAME,tn.CONTACT_NUMBER from PARTY_ROLE as pr join"+
		" PARTY_SUPPLEMENTAL_DATA as  psd on pr.PARTY_ID=psd.PARTY_ID join "+
		" TELECOM_NUMBER as tn on psd.PRIMARY_TELECOM_NUMBER_ID= tn.CONTACT_MECH_ID join "+
		" PERSON as p on p.PARTY_ID=pr.PARTY_ID "+
		" where pr.ROLE_TYPE_ID ='EGATEE_driver' and p.PARTY_ID = 'parameter'";
		int maxshownum=10;
		int index=0;
		List retlist=new ArrayList<Drverinfobean>();
		GenericHelperInfo helperInfo = delegator.getGroupHelperInfo("org.ofbiz");
		SQLProcessor processor = new SQLProcessor(helperInfo);
		try {
			sql=sql.replace("parameter", party);
			ResultSet rs = processor.executeQuery(sql);
			while (rs.next()) {
				String PARTY_ID = rs.getString("PARTY_ID");
				String FIRST_NAME = rs.getString("FIRST_NAME");
				String CONTACT_NUMBER = rs.getString("CONTACT_NUMBER");
				Drverinfobean temp = new Drverinfobean(PARTY_ID, FIRST_NAME, CONTACT_NUMBER);
	...
	public static Map<String, Object> udataedriverandlicesn(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> results = ServiceUtil.returnSuccess();
		...
		String shipmentidlist = request.getParameter("shipmentidlist");
		String driverid = request.getParameter("driverid");
		String license = request.getParameter("license");
		
		String[] templist = shipmentidlist.split("#");
		if(templist.length<=0||driverid==null||"".equals(driverid)||license==null||"".equals(license))
			return ServiceUtil.returnFailure();
		
		String sql="UPDATE SHIPMENT SET DRIVER_ID = \'"+driverid+"\',LICENSE = \'"+license+"\' WHERE ";
		...
		for(int i=0;i<templist.length;i++)
		{
			...
			if(isfirst)
			{
				sql+=" SHIPMENT_ID = \'"+temp+"\'";
				isfirst=false;
			}
			else 
			{
				sql+=" or SHIPMENT_ID = \'"+temp+"\'";
			}
		}
	...
	public static Map<String, Object> fuzzyqueryEGATEEdriver(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> results = ServiceUtil.returnSuccess();
		...
		" TELECOM_NUMBER as tn on psd.PRIMARY_TELECOM_NUMBER_ID= tn.CONTACT_MECH_ID join "+
		" PERSON as p on p.PARTY_ID=pr.PARTY_ID "+
		" where pr.ROLE_TYPE_ID ='EGATEE_driver' and p.FIRST_NAME like '%parameter%'";
		int maxshownum=10;
		int index=0;
		List retlist=new ArrayList<Drverinfobean>();
		Delegator delegator = (Delegator) request.getAttribute("delegator");
		...
		SQLProcessor processor = new SQLProcessor(helperInfo);
		try {
			sql=sql.replace("parameter", firstName);
			ResultSet rs = processor.executeQuery(sql);
			while (rs.next()) {
				String PARTY_ID = rs.getString("PARTY_ID");
				String FIRST_NAME = rs.getString("FIRST_NAME");
		...
		results.put("retlist", retlist);
		return results;
	}
	public static Map<String, Object> CreateEGATEEdriver(HttpServletRequest request, HttpServletResponse response) {
		...
		" TELECOM_NUMBER as tn on psd.PRIMARY_TELECOM_NUMBER_ID= tn.CONTACT_MECH_ID join "+
		" PERSON as p on p.PARTY_ID=pr.PARTY_ID "+
		" where pr.ROLE_TYPE_ID ='EGATEE_driver' and p.FIRST_NAME = \'"+firstName+"\'";
		GenericHelperInfo helperInfo = delegator.getGroupHelperInfo("org.ofbiz");
		SQLProcessor processor = new SQLProcessor(helperInfo);
		try {
			ResultSet rs = processor.executeQuery(sql);
			while (rs.next()) {
				return ServiceUtil.returnError("Already exists named "+firstName+" driver");
			}
        ...
        serviceResults = dispatcher.runSync("createPartyRole", UtilMisc.toMap("partyId", contactPartyId, "roleTypeId", "EGATEE_driver", "userLogin", userLogin));
			if (ServiceUtil.isError(serviceResults)) {
				return serviceResults;
			}
    ...
    public static Map<String, Object> updateShipmentRouteSegment(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        ...
        String carrierServiceStatusId = (String) context.get("carrierServiceStatusId");
        BigDecimal billingWeight = (BigDecimal) context.get("billingWeight");
        String billingWeightUomId = (String) context.get("billingWeightUomId");
        
        Map<String, Object> results = ServiceUtil.returnSuccess();

        try {
        	if(carrierServiceStatusId!=null&&!"".equals(carrierServiceStatusId))
        	{
            GenericValue shipmentRouteSeg = delegator.findByPrimaryKey("ShipmentRouteSegment", UtilMisc.toMap("shipmentId", shipmentId, "shipmentRouteSegmentId", shipmentRouteSegmentId));
            shipmentRouteSeg.set("carrierServiceStatusId", carrierServiceStatusId);
            shipmentRouteSeg.store();
        	}
        ...
        if("SHRSCS_SHIPPED".equals(carrierServiceStatusId))
        {
        	String sql="UPDATE ORDER_HEADER SET ORDER_HEADER.STATUS_ID=\'ORDER_SHIPPED\' "
        		+"	where ORDER_HEADER.ORDER_ID=(select SHIPMENT.PRIMARY_ORDER_ID from SHIPMENT where SHIPMENT.SHIPMENT_ID=\'"+shipmentId+"\')";
        	
        	String sql2="UPDATE SHIPMENT SET SHIPMENT.STATUS_ID=\'SHIPMENT_SHIPPED\' "
			...
			try {
				...
				processor.prepareStatement(sql2);
				processor.executeUpdate();
        ...
        }else if("SHRSCS_NOT_STARTED".equals(carrierServiceStatusId))
        {
            try {
            	List<GenericValue> shipmentReceipts = delegator.findByAnd("ShipmentRouteSegment", UtilMisc.toMap("shipmentId", shipmentId));
            	for(int i=0;i<shipmentReceipts.size();i++)
            	{
            		GenericValue tenmp = shipmentReceipts.get(i);
            		tenmp.set("billingWeight", billingWeight);
            		tenmp.set("billingWeightUomId", billingWeightUomId);
            		tenmp.store();
            	}
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
