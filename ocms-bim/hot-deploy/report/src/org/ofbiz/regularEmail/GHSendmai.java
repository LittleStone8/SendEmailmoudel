...
public class GHSendmai {
     
	
	
	public static Map<String, Object> GH_RegularSendmail(DispatchContext dctx, Map<String, ? extends Object> context) {
...
//		String sql = " select ret.PRODUCT_STORE_ID as productstoreid ,(select PRODUCT_STORE.STORE_NAME from PRODUCT_STORE where PRODUCT_STORE.PRODUCT_STORE_ID=ret.PRODUCT_STORE_ID) as shop,sum(ret.unp) as summary,count(ret.ORDER_ID) as qty from "
//				+ " ( "
//				+ " select oh.*,sum(oi.UNIT_PRICE) as unp from ORDER_HEADER as oh join ORDER_ITEM as oi on oh.ORDER_ID=oi.ORDER_ID  where oh.CREATED_STAMP >=\'"+defaultStartDate+"\'  and  oh.CREATED_STAMP<=\'"+defaultEndDate+"\' and (oh.STATUS_ID='ORDER_COMPLETED' or  oh.STATUS_ID='ORDER_PACKED' or oh.STATUS_ID='ORDER_PAYMENT' or oh.STATUS_ID='ORDER_SHIPPED' or oh.STATUS_ID='ORDER_COMPLETED_W_P' )  group by oi.ORDER_ID "
//				+ " ) as ret  where  ret.PRODUCT_STORE_ID not in  (select PRODUCT_STORE_GROUP_MEMBER.PRODUCT_STORE_ID from PRODUCT_STORE_GROUP_MEMBER where PRODUCT_STORE_GROUP_MEMBER.PRODUCT_STORE_GROUP_ID='TEST')     group by ret.PRODUCT_STORE_ID order by qty asc";
		...
		String sql = "select rrr.PRODUCT_STORE_ID as productstoreid,rrr.STORE_NAME as shop,ret.unp as summary,ret.www as qty from   "
				...
				" (   "
				+ " 	select oh.PRODUCT_STORE_ID,sum(oh.GRAND_TOTAL) as unp,count(oh.GRAND_TOTAL)  as www from ORDER_HEADER as oh where 1=1    and  oh.CREATED_STAMP >=\'"+defaultStartDate+"\'  and  oh.CREATED_STAMP<=\'"+defaultEndDate+"\' and (oh.STATUS_ID='ORDER_COMPLETED' or  oh.STATUS_ID='ORDER_PACKED' or oh.STATUS_ID='ORDER_PAYMENT' or oh.STATUS_ID='ORDER_SHIPPED' or oh.STATUS_ID='ORDER_COMPLETED_W_P' )  GROUP by oh.PRODUCT_STORE_ID "
				+
