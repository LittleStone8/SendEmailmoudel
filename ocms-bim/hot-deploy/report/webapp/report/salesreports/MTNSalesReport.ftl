		...
		<div class="seller_reports_content">
			<div class="seller_reports_choose">
				...
				<div class="seller_reports_choose_content">
					<form id="search_form" class="search_form" action="/report/control/MTN_ExportSalesReport">
						<div class="condition">
							<div class="product_flag">
								<span>Product ID</span> <input type="text" id="productIdInput" name="productId" />
							</div>
	
							<div class="choose_store_group">
								<span>Store Groups</span> 
								<select id="selectGroup" name="sGroupId">
			...
			<div class="seller_reports_detail">
				<div class="seller_reports_first">
					<div class="seller_reports_first_title">
						<span class="seller_reports_first_tit">Sales Details</span> <a
							href="javascript:void(0)" class="export export_myTable">Export</a>
					</div>
				...
				<div class="seller_reports_second">
					<div class="seller_reports_second_title">
						<span class="seller_reports_second_tit">Payment Summary</span> <a
							href="javascript:void(0);" class="export export_myTotal">Export</a>
					</div>
					<div class="seller_reports_tableArea">
    ...
    function showErrorMessage(message){
  	...
  	function initSGroup(){
  		jQuery.ajax({
  			...
  			url: '/report/control/SearchAvailableStoresAndSGroups_MTNSALE',
  			success: function (data) {
  				if(data.responseMessage=="success"){
  ...
  function initTable(){	
   ...
   function searchTable(target){
		if((new Date($("#beginDate").val())) > (new Date($("#endDate").val()))){
			showErrorMessage("The End Date can't beyond the beginning Date");
			return;
		}
		...
		jQuery.ajax({
			...
			url: '/report/control/SearchMarketByConditions_MTN',
			success: function (data) {
		...
		$.each(mydata,function(index,elem){
		    var rowDate = [];
	  		    rowDate.push(elem.orderDate,elem.category,elem.productId,elem.description,elem.quantity,
	  	  		    		elem.currentPrice,elem.specialCostPrice,elem.subTotal,elem.totalSpecialCostPrice,elem.adjustment,elem.payment,
