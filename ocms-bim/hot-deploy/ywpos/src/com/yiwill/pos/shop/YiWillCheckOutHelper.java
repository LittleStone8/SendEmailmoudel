...
/**
 * A facade over the YiWillCart to simplify the relatively complex
 * processing required to create an order in the system.
 */
public class YiWillCheckOutHelper {

    public static final String module = YiWillCheckOutHelper.class.getName();
    ...
    protected YiWillCart cart = null;

    public YiWillCheckOutHelper(Delegator delegator, LocalDispatcher dispatcher, YiWillCart cart) {
    ...
    public List setCheckOutPaymentInternal(Map selectedPaymentMethods, List singleUsePayments, String billingAccountId) {
        ...
        if (UtilValidate.isNotEmpty(selectedPaymentMethods)) {
            ...
            while (i.hasNext()) {
                ...
                YiWillCart.CartPaymentInfo inf = cart.addPaymentAmount(checkOutPaymentId, paymentAmount, singleUse);
    ...
    public Map setCheckOutOptions(String shippingMethod, String shippingContactMechId, Map selectedPaymentMethods,
        ...
        if (UtilValidate.isNotEmpty(this.cart)) {
            ...
            Map shipEstimateMap = YiWillShippingEvents.getShipGroupEstimate(dispatcher, delegator, cart, 0);
    ...
    public Map<String, Object> createOrder(GenericValue userLogin, String distributorId, String affiliateId,
        ...
        //
        Iterator YiWillCartItems = this.cart.items().iterator();
        while (YiWillCartItems.hasNext()) {
            YiWillCartItem YiWillCartItem = (YiWillCartItem)YiWillCartItems.next();
            String requirementId = YiWillCartItem.getRequirementId();
    ...
    public void calcAndAddTax(GenericValue shipAddress) throws GeneralException {
        ...
        for (int i = 0; i < shipGroups; i++) {
            Map YiWillCartItemIndexMap = new HashMap();
            Map serviceContext = this.makeTaxContext(i, shipAddress, YiWillCartItemIndexMap);
            ...
            YiWillCart.CartShipInfo csi = cart.getShipInfo(i);
            ...
            if (itemAdj != null) {
                for (int x = 0; x < itemAdj.size(); x++) {
                    ...
                    YiWillCartItem item = (YiWillCartItem) YiWillCartItemIndexMap.get(Integer.valueOf(x));
                    if (adjs == null) {
                        adjs = new LinkedList();
                    }
    ...
    private Map makeTaxContext(int shipGroup, GenericValue shipAddress, Map YiWillCartItemIndexMap) throws GeneralException {
        YiWillCart.CartShipInfo csi = cart.getShipInfo(shipGroup);
        ...
        Iterator<YiWillCartItem> it = csi.shipItemInfo.keySet().iterator();
        for (int i = 0; i < totalItems; i++) {
            YiWillCartItem cartItem = it.next();
            YiWillCart.CartShipInfo.CartShipItemInfo itemInfo = csi.getShipItemInfo(cartItem);
            ...
            YiWillCartItemIndexMap.put(Integer.valueOf(i), cartItem);
        }

        //add promotion adjustments
        List allAdjustments = cart.getAdjustments();
        BigDecimal orderPromoAmt = YiWillCartOrderReadHelper.calcOrderPromoAdjustmentsBd(allAdjustments);
        ...
        if (shipAddress == null) {
            for (int i = 0; i < cart.selectedPayments(); i++) {
                YiWillCart.CartPaymentInfo cpi = cart.getPaymentInfo(i);
    ...
    public Map<String, Object> processPayment(GenericValue productStore, GenericValue userLogin) throws GeneralException {
        return YiWillCheckOutHelper.processPayment(this.cart.getOrderId(), this.cart.getGrandTotal(), this.cart.getCurrency(), productStore, userLogin, false, false, dispatcher, delegator);
    }
    ...
    public Map<String, Object> processPayment(GenericValue productStore, GenericValue userLogin, boolean faceToFace) throws GeneralException {
        return YiWillCheckOutHelper.processPayment(this.cart.getOrderId(), this.cart.getGrandTotal(), this.cart.getCurrency(), productStore, userLogin, faceToFace, false, dispatcher, delegator);
    }
    ...
    public Map<String, Object> processPayment(GenericValue productStore, GenericValue userLogin, boolean faceToFace, boolean manualHold) throws GeneralException {
        return YiWillCheckOutHelper.processPayment(this.cart.getOrderId(), this.cart.getGrandTotal(), this.cart.getCurrency(), productStore, userLogin, faceToFace, manualHold, dispatcher, delegator);
    }
    ...
    public Map<String, Object> validatePaymentMethods() {
        ...
        if (paymentMethods != null) {
            ...
            while (npi.hasNext()) {
                ...
                YiWillCart.CartPaymentInfo info = cart.getPaymentInfo(paymentMethodId);
