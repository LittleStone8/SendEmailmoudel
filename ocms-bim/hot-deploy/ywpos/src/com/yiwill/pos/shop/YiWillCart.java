 ...
 */
public class YiWillCart implements Iterable<YiWillCartItem>, Serializable {

    public static final String module = YiWillCart.class.getName();
    ...
    private Boolean isNeedShip = false;
...
//    protected long nextItemSeq = 1;
    ...
    protected List<YiWillCartItem> cartLines = FastList.newInstance();
    ...
    protected GenericValue salesUser = null;
    ...
    protected YiWillCart() {}

    /** Creates a new cloned YiWillCart Object. */
    public YiWillCart(YiWillCart cart) {
        ...
        while (itIt.hasNext()) {
            cartLines.add(new YiWillCartItem((YiWillCartItem) itIt.next()));
        }
    }

    /** Creates new empty YiWillCart object. */
    public YiWillCart(Delegator delegator, String productStoreId,String productStoreGroupId, String webSiteId, Locale locale, String currencyUom, String billToCustomerPartyId, String billFromVendorPartyId) {
    ...
    /** Creates new empty YiWillCart object. */
    public YiWillCart(Delegator delegator, String productStoreId, String productStoreGroupId,String webSiteId, Locale locale, String currencyUom) {
        this(delegator, productStoreId,productStoreGroupId, webSiteId, locale, currencyUom, null, null);
    }

//    /** Creates a new empty YiWillCart object. */
//    public YiWillCart(Delegator delegator, String productStoreId, Locale locale, String currencyUom) {
...
//    }
    ...
    public Boolean getIsNeedShip() {
		return isNeedShip;
	}

	public void setIsNeedShip(Boolean isNeedShip) {
		this.isNeedShip = isNeedShip;
	}
    ...
    public void setCurrency(LocalDispatcher dispatcher, String currencyUom) throws Exception {
        if (isReadOnlyCart()) {
           throw new Exception("Cart items cannot be changed");
        }
        ...
        if (!previousCurrency.equals(this.currencyUom)) {
            ...
            while (itemIterator.hasNext()) {
                YiWillCartItem item = (YiWillCartItem) itemIterator.next();
                item.updatePrice(dispatcher, this);
            }
        }
    }
    
    public void reCountPrice(LocalDispatcher dispatcher) throws Exception {
        if (isReadOnlyCart()) {
           throw new Exception("Cart items cannot be changed");
        }
        Iterator itemIterator = this.iterator();
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
            ...
            ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber, String parentProductId, LocalDispatcher dispatcher) throws Exception {
       ...
       return addOrIncreaseItem(productId,selectedAmount,quantity,reservStart,reservLength,reservPersons,
                ...
                configWrapper,itemType,itemGroupNumber,parentProductId,dispatcher,null);
    }
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
            ...
            Timestamp shipBeforeDate, Timestamp shipAfterDate, Map features, Map attributes, String prodCatalogId,
            ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber, String parentProductId, LocalDispatcher dispatcher,String imei) throws Exception {
        if (isReadOnlyCart()) {
           throw new Exception("Cart items cannot be changed");
        }
        ...
        YiWillCart.YiWillCartItemGroup itemGroup = this.getItemGroupByNumber(itemGroupNumber);
        GenericValue supplierProduct = null;
        // Check for existing cart item.
        for (int i = 0; i < this.cartLines.size(); i++) {
            YiWillCartItem sci = (YiWillCartItem) cartLines.get(i);

            String cartImei = sci.getImei();
            if(imei != null && imei.equals(cartImei) && !"".equals(imei)){
            	throw new Exception("This IMEI product has already existed in the cart.");
            }

            if ((imei == null || "".equals(imei)) && sci.baseEquals(productId,false) && (cartImei == null || "".equals(cartImei))) {
                ...
                if (sci.getItemType().equals("RENTAL_ORDER_ITEM")) {
                    ...
                    String isAvailable = YiWillCartItem.checkAvailability(productId, newQuantity, reservStart, reservLength, this);
                    if (isAvailable.compareTo("OK") != 0) {
                        Map messageMap = UtilMisc.toMap("productId", productId, "availableMessage", isAvailable);
                        String excMsg = UtilProperties.getMessage(YiWillCartItem.resource, "item.product_not_available", messageMap, this.getLocale());
...
//                        sci.setName(YiWillCartItem.getPurchaseOrderItemDescription(sci.getProduct(), supplierProduct, this.getLocale()));
//                    } else {
...
//                 return this.addItem(0, YiWillCartItem.makePurchaseOrderItem(Integer.valueOf(0), productId, selectedAmount, quantity, features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher, this, supplierProduct, shipBeforeDate, shipAfterDate, cancelBackOrderDate));
//            } else {
//                throw new Exception("SupplierProduct not found");
//            }
//        } else {
...
//                Debug.logError(e, module);
//            }
            return this.addItem(0, YiWillCartItem.makeItem(Integer.valueOf(0), productId, selectedAmount, quantity, null,
                    ...
                    this, Boolean.TRUE, Boolean.TRUE, parentProductId, Boolean.FALSE, Boolean.FALSE,imei));
//        }
    }
    ...
    public int addNonProductItem(String itemType, String description, String categoryId, BigDecimal price, BigDecimal quantity,
            Map attributes, String prodCatalogId, String itemGroupNumber, LocalDispatcher dispatcher) throws Exception {
        YiWillCart.YiWillCartItemGroup itemGroup = this.getItemGroupByNumber(itemGroupNumber);
        return this.addItem(0, YiWillCartItem.makeItem(Integer.valueOf(0), itemType, description, categoryId, price, null, quantity, attributes, prodCatalogId, itemGroup, dispatcher, this, Boolean.TRUE));
    }

    /** Add an item to the shopping cart. */
    public int addItem(int index, YiWillCartItem item) throws Exception {
        if (isReadOnlyCart()) {
           throw new Exception("Cart items cannot be changed");
        }
            ...
            if (product != null && isSalesOrder()) {
                ...
                if (billingAddress != null) {
                    if (!ProductWorker.isBillableToAddress(product, billingAddress)) {
                        throw new Exception("The billing address is not compatible with ProductGeos rules of this product.");
                    }
                }
            }
    ...
    /** Add an item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap features, HashMap attributes, String prodCatalogId, String itemType, ProductConfigWrapper configWrapper, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, null, null, null, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, Boolean.FALSE, Boolean.FALSE));
    }

    /** Add an item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap features, HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws Exception, Exception {
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap features, HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, Boolean.FALSE, Boolean.FALSE));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap features, HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, skipInventoryChecks, skipProductChecks));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, HashMap features, HashMap attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, skipInventoryChecks, skipProductChecks));
    }
    ...
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId, HashMap features, HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, Boolean.FALSE, Boolean.FALSE,null));
    }

    /** Add an accommodation(rental)item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId, HashMap features, HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, skipInventoryChecks, skipProductChecks,null));
    }

    /** Add an accommodation(rental/aggregated)item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersonsDbl,String accommodationMapId, String accommodationSpotId, HashMap features, HashMap attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks,String imei) throws Exception, Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, reservStart, reservLength, reservPersonsDbl, accommodationMapId, accommodationSpotId, null, null, features, attributes, prodCatalogId, configWrapper, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, skipInventoryChecks, skipProductChecks,imei));
    }

    /** Add an item to the shopping cart. */
    public int addItemToEnd(String productId, BigDecimal amount, BigDecimal quantity, BigDecimal unitPrice, HashMap features, HashMap attributes, String prodCatalogId, String itemType, LocalDispatcher dispatcher, Boolean triggerExternalOps, Boolean triggerPriceRules, Boolean skipInventoryChecks, Boolean skipProductChecks) throws Exception {
        return addItemToEnd(YiWillCartItem.makeItem(null, productId, amount, quantity, unitPrice, null, null, null, null, null, features, attributes, prodCatalogId, null, itemType, null, dispatcher, this, triggerExternalOps, triggerPriceRules, null, skipInventoryChecks, skipProductChecks));
    }

    /** Add an item to the shopping cart. */
    public int addItemToEnd(YiWillCartItem item) throws Exception {
        return addItem(cartLines.size(), item);
    }

    /** Get a YiWillCartItem from the cart object. */
    public YiWillCartItem findCartItem(String productId, Map features, Map attributes, String prodCatalogId, BigDecimal selectedAmount) {
        // Check for existing cart item.
        for (int i = 0; i < this.cartLines.size(); i++) {
            YiWillCartItem cartItem = (YiWillCartItem) cartLines.get(i);
    ...
    /** Get all YiWillCartItems from the cart object with the given productId. */
    public List findAllCartItems(String productId) {
        return this.findAllCartItems(productId, null);
    }
    /** Get all YiWillCartItems from the cart object with the given productId and optional groupNumber to limit it to a specific item group */
    public List findAllCartItems(String productId, String groupNumber) {
        ...
        // Check for existing cart item.
        for (YiWillCartItem cartItem : cartLines) {
    ...
    /** Get all YiWillCartItems from the cart object with the given productCategoryId and optional groupNumber to limit it to a specific item group */
    ...
    /** Remove quantity 0 YiWillCartItems from the cart object. */
    ...
    public void removeExtraItems(List multipleItems, LocalDispatcher dispatcher, int maxItems) throws Exception {
        ...
        while (localIter.hasNext()) {
            YiWillCartItem item = (YiWillCartItem) localIter.next();
    ...
    public void ensureItemsQuantity(List cartItems, LocalDispatcher dispatcher, BigDecimal quantity) throws Exception {
        Iterator localIter = cartItems.iterator();
        while (localIter.hasNext()) {
            YiWillCartItem item = (YiWillCartItem) localIter.next();
    ...
    public BigDecimal ensureItemsTotalQuantity(List cartItems, LocalDispatcher dispatcher, BigDecimal quantity) throws Exception {
    ...
    public boolean containOnlyDigitalGoods(int shipGroupIdx) {
        ...
        for (YiWillCartItem cartItem: shipInfo.getShipItems()) {
            GenericValue product = cartItem.getProduct();
            try {
                GenericValue productType = product.getRelatedOneCache("ProductType");
                if (productType == null || !"N".equals(productType.getString("isPhysical"))) {
    ...
    public int getItemIndex(YiWillCartItem item) {
        return cartLines.indexOf(item);
    }

    /** Get a YiWillCartItem from the cart object. */
    public YiWillCartItem findCartItem(int index) {
        if (cartLines.size() <= index) {
            return null;
        }
        return (YiWillCartItem) cartLines.get(index);

    }

    public YiWillCartItem findCartItem(String orderItemSeqId) {
    ...
    public void removeCartItem(YiWillCartItem item, LocalDispatcher dispatcher) throws Exception {
        if (item == null) return;
        this.removeCartItem(this.getItemIndex(item), dispatcher);
    }

    /** Remove an item from the cart object. */
    public void removeCartItem(int index, LocalDispatcher dispatcher) throws Exception {
        if (isReadOnlyCart()) {
           throw new Exception("Cart items cannot be changed");
        }
        if (index < 0) return;
        if (cartLines.size() <= index) return;
        YiWillCartItem item = (YiWillCartItem) cartLines.remove(index);
    ...
    public List<YiWillCartItem> items() {
        List<YiWillCartItem> result = FastList.newInstance();
        result.addAll(cartLines);
        return result;
    }

    /** Returns an iterator of cart items. */
    public Iterator<YiWillCartItem> iterator() {
        return cartLines.iterator();
    }

    public YiWillCart.YiWillCartItemGroup getItemGroupByNumber(String groupNumber) {
        if (UtilValidate.isEmpty(groupNumber)) return null;
        return (YiWillCart.YiWillCartItemGroup) this.itemGroupByNumberMap.get(groupNumber);
    }
    ...
    public String addItemGroup(String groupName, String parentGroupNumber) {
        YiWillCart.YiWillCartItemGroup parentGroup = this.getItemGroupByNumber(parentGroupNumber);
        YiWillCart.YiWillCartItemGroup newGroup = new YiWillCart.YiWillCartItemGroup(this.nextGroupNumber, groupName, parentGroup);
        this.nextGroupNumber++;
        this.itemGroupByNumberMap.put(newGroup.getGroupNumber(), newGroup);
        return newGroup.getGroupNumber();
    }

    public YiWillCartItemGroup addItemGroup(GenericValue itemGroupValue) throws GenericEntityException {
        if (itemGroupValue == null) {
            return null;
        }
        String itemGroupNumber = itemGroupValue.getString("orderItemGroupSeqId");
        YiWillCartItemGroup itemGroup = this.getItemGroupByNumber(itemGroupNumber);
        if (itemGroup == null) {
            YiWillCartItemGroup parentGroup = addItemGroup(itemGroupValue.getRelatedOneCache("ParentOrderItemGroup"));
            itemGroup = new YiWillCartItemGroup(itemGroupNumber, itemGroupValue.getString("groupName"), parentGroup);
    ...
    public List getCartItemsInGroup(String groupNumber) {
        List cartItemList = FastList.newInstance();
        YiWillCart.YiWillCartItemGroup itemGroup = this.getItemGroupByNumber(groupNumber);
        if (itemGroup != null) {
            for (YiWillCartItem cartItem : cartLines) {
    ...
    public void deleteItemGroup(String groupNumber) {
        YiWillCartItemGroup itemGroup = this.getItemGroupByNumber(groupNumber);
        if (itemGroup != null) {
            ...
            while (cartItemIter.hasNext()) {
                YiWillCartItem cartItem = (YiWillCartItem) cartItemIter.next();
            ...
            while (itemGroupIter.hasNext()) {
                YiWillCartItemGroup otherItemGroup = (YiWillCartItemGroup) itemGroupIter.next();
    ...
    public GenericValue getSalesUser() {
  		return salesUser;
  	}
    
    public void setSalesUser(GenericValue salesUser){
    	 this.salesUser = salesUser;
    }
    
    public void setUserLogin(GenericValue userLogin, LocalDispatcher dispatcher) throws Exception {
    ...
    public void setAutoUserLogin(GenericValue autoUserLogin, LocalDispatcher dispatcher) throws Exception {
    ...
    public void handleNewUser(LocalDispatcher dispatcher) throws Exception {
        ...
        if (UtilValidate.isNotEmpty(partyId)) {
            ...
            while (promoCodeIter.hasNext()) {
                ...
                String checkResult = YiWillProductPromoWorker.checkCanUsePromoCode(promoCode, partyId, this.getDelegator(), locale);
                if (checkResult != null) {
            ...
            // rerun promotions
            YiWillProductPromoWorker.doPromotions(this, dispatcher);
        }
    }
...
//                Iterator products = (YiWillCart.getItemsProducts(this.cartLines)).iterator();
...
//	return this.addPaymentAmount(id, amount, null, null, isSingleUse, false, true);
        return this.addPaymentAmount(id, amount, null, null, isSingleUse, false, false);
    }
    ...
    public BigDecimal getOrderShipping() {
        return YiWillCartOrderReadHelper.calcOrderAdjustments(this.getAdjustments(), this.getSubTotal(), false, false, true);
    }
    ...
    public Map getShipGroups(YiWillCartItem item) {
    ...
    /** Returns the YiWillCartItem (key) and quantity (value) associated with the ship group */
    public Map<YiWillCartItem, BigDecimal> getShipGroupItems(int idx) {
        CartShipInfo csi = this.getShipInfo(idx);
        Map<YiWillCartItem, BigDecimal> qtyMap = FastMap.newInstance();
        for(YiWillCartItem item : csi.shipItemInfo.keySet()) {
            CartShipInfo.CartShipItemInfo csii = csi.shipItemInfo.get(item);
    ...
    public void clearItemShipInfo(YiWillCartItem item) {
     ...
     * @param item
     */
    public void setShipGroupShipDatesFromItem(YiWillCartItem item) {
    ...
    public void setItemShipGroupQty(int itemIndex, BigDecimal quantity, int idx) {
        YiWillCartItem itemIdx = this.findCartItem(itemIndex);
        if(itemIdx != null) {
            this.setItemShipGroupQty(itemIdx, itemIndex, quantity, idx);
        }
    }

    public void setItemShipGroupQty(YiWillCartItem item, BigDecimal quantity, int idx) {
        this.setItemShipGroupQty(item, this.getItemIndex(item), quantity, idx);
    }

    public void setItemShipGroupQty(YiWillCartItem item, int itemIndex, BigDecimal quantity, int idx) {
        if (itemIndex > -1) {
            CartShipInfo csi = this.getShipInfo(idx);
    ...
    public BigDecimal getItemShipGroupQty(YiWillCartItem item, int idx) {
        if (item != null) {
            CartShipInfo csi = this.getShipInfo(idx);
            CartShipInfo.CartShipItemInfo csii = csi.shipItemInfo.get(item);
            if (csii != null) {
    ...
    public void positionItemToGroup(YiWillCartItem item, BigDecimal quantity, int fromIndex, int toIndex, boolean clearEmptyGroups) {
    ...
    public void cleanUpShipGroups() {
        for (CartShipInfo csi : this.shipInfo) {
            Iterator<YiWillCartItem> si = csi.shipItemInfo.keySet().iterator();
            while (si.hasNext()) {
                YiWillCartItem item = si.next();
        ...
        if (isSalesOrder() && UtilValidate.isNotEmpty(shippingContactMechId)) {
            ...
            if (shippingAddress != null) {
                ...
                if (UtilValidate.isNotEmpty(shipItems)) {
                    ...
                    while (siit.hasNext()) {
                        YiWillCartItem cartItem = (YiWillCartItem) siit.next();
                        GenericValue product = cartItem.getProduct();
...
//            ShippingEstimateWrapper shipEstimateWrapper = org.ofbiz.order.YiWillCart.shipping.ShippingEstimateWrapper.getWrapper(dispatcher, this, 0);
...
//            }
    ...
    public BigDecimal getItemTotal() {
        ...
        while (i.hasNext()) {
            itemTotal = itemTotal.add(((YiWillCartItem) i.next()).getBasePrice());
        }
        return itemTotal;
    }
    ...
    public BigDecimal getSubTotal() {
        ...
        while (i.hasNext()) {
            itemsTotal = itemsTotal.add(((YiWillCartItem) i.next()).getItemSubTotal());
        }
        return itemsTotal;
    }
    ...
    public BigDecimal getDisplaySubTotal() {
        ...
        Iterator i = iterator();
        while (i.hasNext()) {
            itemsTotal = itemsTotal.add(((YiWillCartItem) i.next()).getDisplayItemSubTotal());
        }
        return itemsTotal;
    }
    ...
    public BigDecimal getDisplayRecurringSubTotal() {
        ...
        Iterator i = iterator();
        while (i.hasNext()) {
            itemsTotal = itemsTotal.add(((YiWillCartItem) i.next()).getDisplayItemRecurringSubTotal());
        }
        return itemsTotal;
    }
    ...
    public BigDecimal getOrderOtherAdjustmentTotal() {
        return YiWillCartOrderReadHelper.calcOrderAdjustments(this.getAdjustments(), this.getSubTotal(), true, false, false);
    }
    ...
    public BigDecimal getSubTotalForPromotions() {
        ...
        while (i.hasNext()) {
            YiWillCartItem cartItem = (YiWillCartItem) i.next();
            GenericValue product = cartItem.getProduct();
    ...
    public void removeAdjustmentByType(String orderAdjustmentTypeId) {
        ...
        while (cartIterator.hasNext()) {
            YiWillCartItem item = (YiWillCartItem) cartIterator.next();
    ...
    public BigDecimal getTotalWeight() {
        ...
        while (i.hasNext()) {
            YiWillCartItem item = (YiWillCartItem) i.next();
    ...
    public BigDecimal getTotalQuantity() {
        ...
        while (i.hasNext()) {
            count = count.add(((YiWillCartItem) i.next()).getQuantity());
        }
        return count;
    }
    ...
    public BigDecimal getShippableTotal(int idx) {
        ...
        for (YiWillCartItem item : info.shipItemInfo.keySet()) {
...
//    }
    ...
    public void clearCartItemUseInPromoInfo() {
        ...
        while (cartLineIter.hasNext()) {
            YiWillCartItem cartLine = (YiWillCartItem) cartLineIter.next();
    ...
    public BigDecimal getProductPromoTotal() {
        ...
        Iterator cartItemIter = this.iterator();
        while (cartItemIter.hasNext()) {
            YiWillCartItem checkItem = (YiWillCartItem) cartItemIter.next();
    ...
    public void clearAllItemStatus() {
        ...
        while (lineIter.hasNext()) {
            YiWillCartItem item = (YiWillCartItem) lineIter.next();
    ...
    public String addProductPromoCode(String productPromoCodeId, LocalDispatcher dispatcher) {
        ...
        String checkResult = YiWillProductPromoWorker.checkCanUsePromoCode(productPromoCodeId, this.getPartyId(), this.getDelegator(), this, locale);
        if (checkResult == null) {
    ...
    public synchronized void resetPromoRuleUse(String productPromoId, String productPromoRuleId) {
        Iterator lineIter = this.iterator();
        while (lineIter.hasNext()) {
            YiWillCartItem cartItem = (YiWillCartItem) lineIter.next();
     ...
     * @param dispatcher
     */
    public String getAggregatedInstanceId (YiWillCartItem item, LocalDispatcher dispatcher) {
    ...
    public List makeOrderItemGroups() {
        ...
        while (groupValueIter.hasNext()) {
            YiWillCart.YiWillCartItemGroup itemGroup = (YiWillCart.YiWillCartItemGroup) groupValueIter.next();
    ...
    protected void explodeItems(LocalDispatcher dispatcher) {
        ...
        synchronized (cartLines) {
            ...
            while (itemIter.hasNext()) {
                YiWillCartItem item = (YiWillCartItem) itemIter.next();
     ...
     *
     * @param YiWillCartItems
     * @param dispatcher
     */
    public void explodeItems(List YiWillCartItems, LocalDispatcher dispatcher) {
        if (dispatcher == null) return;
        synchronized (cartLines) {
            Iterator itemIter = YiWillCartItems.iterator();
            while (itemIter.hasNext()) {
                YiWillCartItem item = (YiWillCartItem) itemIter.next();
    ...
    public List<GenericValue> makeOrderItems(boolean explodeItems, boolean replaceAggregatedId, LocalDispatcher dispatcher) {
        ...
        synchronized (cartLines) {
            ...
            Collections.sort(cartLines, new Comparator() {
                public int compare(Object a, Object b) {
                     String one = ((YiWillCartItem) a).getOrderItemSeqId();
                     String two = ((YiWillCartItem) b).getOrderItemSeqId();
                     
                     BigDecimal oneB = one != null ? new BigDecimal(one) :BigDecimal.ZERO;
                     BigDecimal twoB = two != null ? new BigDecimal(two):BigDecimal.ZERO;
                    return oneB.compareTo(twoB);
                }
            });
            
            for (YiWillCartItem item : cartLines) {
...
//                    String orderItemSeqId = UtilFormatOut.formatPaddedNumber(nextItemSeq, 5);
                ...
                try {
                	GenericValue  product = delegator.findByPrimaryKey("Product", UtilMisc.toMap("productId", item.getProductId()));
                	
                	if(product != null){
                		
                		List<String> names=new ArrayList<String>();
						List<GenericValue> productFeatureAndAppl = product.getRelated("ProductFeatureAndAppl");
						
						for (GenericValue productFeature : productFeatureAndAppl) {
//							GenericValue s = productFeature.getRelatedOne("ProductFeature");
							names.add(productFeature.getString("description"));
						}
						
						String description = StringUtils.join(names, ",");
						//
						description = product.getString("internalName") + " " + description;
						  orderItem.set("itemDescription",description);
                	}
    			...
    			priceContext.put("currencyUomId", getCurrency());
    			priceContext.put("productStoreGroupId", getProductStoreGroupId());
    			...
    			priceContext.put("webSiteId", getWebSiteId());
    			priceContext.put("productStoreId",getProductStoreId());
    			...
    			orderItem.set("unitRecurringPrice",priceResult.get("basePrice"));
...
//                orderItem.set("itemDescription", item.getName());
                ...
                orderItem.set("imei", item.getImei());
    ...
    /** create WorkEfforts from the YiWillCart items when itemType = RENTAL_ORDER_ITEM */
...
//            BigDecimal billingAccountAvailableAmount = YiWillCheckOutHelper.availableAccountBalance(getBillingAccountId(), dispatcher);
...
//            }
            ...
            * of the cart: this kind of implementation is due by the fact that CartPaymentInfo is an inner class.*/
            allOpPrefs.addAll(inf.makeOrderPaymentInfosHandlingChangeDue(delegator, inf.amount.doubleValue()));
//            if((this.getGrandTotal().subtract(this.getPaymentTotal())).compareTo(BigDecimal.ZERO) < 0 ){
...
//            }
    ...
    /** make a list of OrderItemPriceInfos from the YiWillCartItems */
    ...
    /** make a list of OrderContactMechs from the YiWillCart and the YiWillCartItems */
...
//        result.put("orderInternalNotes", this.getInternalOrderNotes());
        ...
        result.put("grandTotal", this.getGrandTotal());
     ...
     * @throws Exception
     */
    public void createDropShipGroups(LocalDispatcher dispatcher) throws Exception {

        // Retrieve the facilityId from the cart's productStoreId because YiWillCart.setFacilityId() doesn't seem to be used anywhere
        ...
        // Intermediate structure supplierPartyId -> { YiWillCartItem = { originalShipGroupIndex = dropShipQuantity } } to collect drop-shippable items
        ...
        while (dsit.hasNext()) {
            ...
            while (itit.hasNext()) {
                ...
                YiWillCartItem cartItem = (YiWillCartItem) itit.next();
                Map cartItemGroupQuantities = (Map) supplierCartItems.get(cartItem);
    ...
    static class BasePriceOrderComparator implements Comparator, Serializable {
        ...
        public int compare(java.lang.Object obj, java.lang.Object obj1) {
            YiWillCartItem cartItem = (YiWillCartItem) obj;
            YiWillCartItem cartItem1 = (YiWillCartItem) obj1;
    ...
    public static class YiWillCartItemGroup implements Serializable {
        private String groupNumber;
        private String groupName;
        private YiWillCartItemGroup parentGroup;

        // don't allow empty constructor
        private YiWillCartItemGroup() {}

        protected YiWillCartItemGroup(long groupNumber, String groupName) {
        ...
        protected YiWillCartItemGroup(long groupNumber, String groupName, YiWillCartItemGroup parentGroup) {
            this(UtilFormatOut.formatPaddedNumber(groupNumber, 2), groupName, parentGroup);
        }

        protected YiWillCartItemGroup(String groupNumber, String groupName, YiWillCartItemGroup parentGroup) {
            this.groupNumber = groupNumber;
            this.groupName = groupName;
            this.parentGroup = parentGroup;
        }

        protected YiWillCartItemGroup(YiWillCartItemGroup itemGroup, YiWillCartItemGroup parentGroup) {
        ...
        public YiWillCartItemGroup getParentGroup () {
        ...
        public boolean equals(Object obj) {
            ...
            YiWillCartItemGroup that = (YiWillCartItemGroup) obj;
    ...
    public static class CartShipInfo implements Serializable {
        public Map<YiWillCartItem, CartShipItemInfo> shipItemInfo = FastMap.newInstance();
	...
	public String getCarrierPartyId() { return carrierPartyId; }
        ...
        public List makeItemShipGroupAndAssoc(Delegator delegator, YiWillCart cart, long groupIndex) {
            ...
            System.out.println("#### "+ this.internalContactMechId);
            ...
            if(this.internalContactMechId != null && this.internalContactMechId.length() >0){
            ...
            for (YiWillCartItem item : shipItemInfo.keySet()) {
        ...
        public CartShipItemInfo setItemInfo(YiWillCartItem item, BigDecimal quantity, List taxAdj) {
        ...
        public CartShipItemInfo setItemInfo(YiWillCartItem item, List taxAdj) {
            return setItemInfo(item, BigDecimal.ONE.negate(), taxAdj);
        }

        public CartShipItemInfo setItemInfo(YiWillCartItem item, BigDecimal quantity) {
            return setItemInfo(item, quantity, null);
        }

        public CartShipItemInfo getShipItemInfo(YiWillCartItem item) {
            return shipItemInfo.get(item);
        }

        public Set<YiWillCartItem> getShipItems() {
            return shipItemInfo.keySet();
        }

        private boolean isShippableToAddress(YiWillCartItem item) {
        ...
        public BigDecimal getTotalTax(YiWillCart cart) {
            ...
            Map<String, Object> taxByAuthority = YiWillCartOrderReadHelper.getOrderTaxByTaxAuthGeoAndParty(taxAdjustments);
        ...
        public static class CartShipItemInfo implements Serializable {
            ...
            public YiWillCartItem item = null;
            public BigDecimal quantity = BigDecimal.ZERO;

            public BigDecimal getItemTax(YiWillCart cart) {
                BigDecimal itemTax = ZERO;

                for (int i = 0; i < itemTaxAdj.size(); i++) {
                    GenericValue v = (GenericValue) itemTaxAdj.get(i);
                    itemTax = itemTax.add(YiWillCartOrderReadHelper.calcItemAdjustment(v, quantity, item.getBasePrice()));
                }

                return itemTax.setScale(taxCalcScale, taxRounding);
            }

            public YiWillCartItem getItem() {
                return this.item;
            }
    ...
    public static class CartPaymentInfo implements Serializable, Comparable {
        ...
        public List makeOrderPaymentInfos(Delegator delegator, YiWillCart cart) {
