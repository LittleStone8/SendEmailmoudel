...
public final class UtilProduct {
    ...
    */
    ...
    @SuppressWarnings("unchecked")
    public static String finimeibyitem(Delegator delegator, List<OrderItemShipGrpInvRes> shipGroupInventoryReservations) throws GenericEntityException {
    	if(shipGroupInventoryReservations.size()!=1)
    		return "----";
    	String inventoryItemId = (String)shipGroupInventoryReservations.get(0).get("inventoryItemId");
    	String sql="select * from INVENTORY_ITEM where INVENTORY_ITEM.INVENTORY_ITEM_ID=\'"+inventoryItemId+"\'";
		...
		try {
			while (rs.next()) {
				...
				String retstr = rs.getString("SERIAL_NUMBER");
