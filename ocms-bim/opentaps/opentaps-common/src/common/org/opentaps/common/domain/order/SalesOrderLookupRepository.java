...
import org.opentaps.gwt.common.server.lookup.ProductStoreLookupService;
...
public class SalesOrderLookupRepository extends CommonLookupRepository implements SalesOrderLookupRepositoryInterface {
    ...
    public List<OrderViewForListing> findOrders() throws RepositoryException {
        ...
        try {
            ...
            //外关联
            //Criteria roleCriteria = criteria.createAlias("orderRoles", "or", Criteria.LEFT_JOIN);
            ...
            if (UtilValidate.isNotEmpty(customerPartyId)) {
            	criteria.add(Restrictions.ilike(OrderHeader.Fields.billToPartyId.name(), customerPartyId));
            }

            // filter by the given purchaseOrderId string, from the OrderItem entity
            //外关联
            criteria.createAlias("orderItems", "oi", Criteria.LEFT_JOIN);
            ...
            if (UtilValidate.isNotEmpty(shipment)) {
            	if(shipment.equals("None")){
            		criteria.add(Restrictions.isNull("oi." +OrderItem.Fields.shipAfterDate.name()));
            	}
            	if(shipment.equals("Need Shipment")){
            		criteria.add(Restrictions.isNotNull("oi." +OrderItem.Fields.shipAfterDate.name()));
            	}
            }
            ...
            //外关联
            criteria.createAlias("orderItemShipGroups", "oisg", Criteria.LEFT_JOIN);
            //外关联
        	Criteria address = criteria.createCriteria("oisg.postalAddress", "postaladdr", Criteria.LEFT_JOIN);
            ...
            criteria.setProjection(Projections.projectionList()
                                   ...
                                   .add(Projections.property(OrderHeader.Fields.billToPartyId.name()))
    ...
    public List<OrderViewForListing> findOrders(HttpServletRequest request) throws RepositoryException {
        ...
        try {
           ...
           // Criteria roleCriteria = criteria.createAlias("orderRoles", "or", Criteria.LEFT_JOIN);
            ...
            boolean flag = security.hasEntityPermission("WRHS", "_" + "ADMIN", request.getSession());
            // filter by product store if given
            if (UtilValidate.isNotEmpty(productStoreId)) {
                criteria.add(Restrictions.eq(OrderHeader.Fields.productStoreId.name(), productStoreId));
            }else if(request.getRequestURI().contains("warehouse")){
                ...
                List<ProductStore> list =  findList(ProductStore.class, EntityCondition.makeCondition("inventoryFacilityId", EntityOperator.EQUALS, facilityId));
                List<String> storeNames = new ArrayList<String>();
                for(ProductStore productStore : list){
                	String storeName = productStore.getProductStoreId();
                	storeNames.add(storeName);
                }
                criteria.add(Restrictions.in(OrderHeader.Fields.productStoreId.name(), storeNames));
            }else if(!flag){
                ...
                if(productStoreId.size() > 0) {
                	criteria.add(Restrictions.in(OrderHeader.Fields.productStoreId.name(), productStoreId));
                }else{
                	return new ArrayList<OrderViewForListing>();
                }
            }
            ...
            // filter by the given customerPartyId string, from the OrderRole entity
            if (UtilValidate.isNotEmpty(customerPartyId)) {
            	//criteria.add(Restrictions.ilike("or.id." + OrderRole.Fields.partyId.name(), customerPartyId, MatchMode.START));
            	criteria.add(Restrictions.ilike(OrderHeader.Fields.billToPartyId.name(), customerPartyId));
            }
            ...
            if (UtilValidate.isNotEmpty(shipment)) {
            	if(shipment.equals("None")){
            		criteria.add(Restrictions.isNull("oisg." +OrderItemShipGroup.Fields.shipAfterDate.name()));
            	}
            	if(shipment.equals("Need Shipment")){
            		criteria.add(Restrictions.isNotNull("oisg." +OrderItemShipGroup.Fields.shipAfterDate.name()));
            	}
            }
            
            //is fix order
            if(isFixOrder == null || "N".equals(isFixOrder)){
            	criteria.add(Restrictions.or(
                         Restrictions.eq(OrderHeader.Fields.isFixOrder.name() , "N"),
                         Restrictions.isNull(OrderHeader.Fields.isFixOrder.name())));
            }else if("Y".equals(isFixOrder)){
            	criteria.add(Restrictions.eq(OrderHeader.Fields.isFixOrder.name() , isFixOrder));
            }
            
            //外关联
            criteria.createAlias("orderPaymentPreferences", "opp", Criteria.LEFT_JOIN);
            if (UtilValidate.isNotEmpty(paymentWay)) {
            		criteria.add(Restrictions.ilike("opp." + OrderPaymentPreference.Fields.paymentMethodTypeId.name(), paymentWay, MatchMode.START));
            }
            //外关联
        	Criteria address = criteria.createCriteria("oisg.postalAddress", "postaladdr", Criteria.LEFT_JOIN);
            if (UtilValidate.isNotEmpty(shippingAddress)) {
            ...
            criteria.setProjection(Projections.projectionList()
                                   ...
                                   .add(Projections.property(OrderHeader.Fields.grandTotal.name()))
                                   .add(Projections.property(OrderHeader.Fields.orderDate.name()))
//                                   .add(Projections.property(OrderHeader.Fields.entryDate.name()))
                                   .add(Projections.property(OrderHeader.Fields.currencyUom.name()))
                                   .add(Projections.property(OrderHeader.Fields.billToPartyId.name()))
                                   .add(Projections.property("oi." + OrderItem.Fields.correspondingPoId.name()))
                                   .add(Projections.property("tco." + TrackingCodeOrder.Fields.trackingCodeId.name()))
            ...
            // set the order by
            if (orderBy == null) {
//                orderBy = Arrays.asList(OrderHeader.Fields.entryDate.desc());
                orderBy = Arrays.asList(OrderHeader.Fields.orderDate.desc());
            }
            // some substitution is needed to fit the hibernate field names
            // this also maps the calculated fields and indicates the non sortable fields
            ...
            //subs.put("partyName", "or.id.partyId");
            subs.put("orderDateString", "orderDate");
//            subs.put("orderDateString", "entryDate");
            subs.put("shipByDateString", null);
            subs.put("orderNameId", "orderId");
            subs.put("statusDescription", "statusId");
            subs.put("correspondingPoId", "oi.correspondingPoId");
            ...
            try {
                ...
                while (o != null) {
                    ...
                    int i = 0;
                    r.setOrderId((String) o[i++]);
                    r.setStorName(getProductStoreName(r.getOrderId(),request));
                    r.setIsFixOrder(isFixOrder(r.getOrderId(),request));
                    r.setOrderName((String) o[i++]);
                    r.setStatusId((String) o[i++]);
                    r.setGrandTotal((BigDecimal) o[i++]);
                    r.setOrderDate((Timestamp) o[i++]);
    ...
    private String getProductStoreName(String orderId,HttpServletRequest request){
    	Delegator delegator = (Delegator) request.getAttribute("delegator");
    	String productStoreName = "";
		try {
			GenericValue orderHead = delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId",orderId));
			String productStoreId = orderHead.getString("productStoreId");
    ...
    public String isFixOrder(String orderId,HttpServletRequest request){
    	Delegator delegator = (Delegator) request.getAttribute("delegator");
    	String isFixOrder = "";
    	try {
			GenericValue orderHead = delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId",orderId));
			isFixOrder = orderHead.getString("isFixOrder");
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    	if("N".equals(isFixOrder) || isFixOrder==null){
    		return "-";
    	}else{
    		return "Y";
    	}
