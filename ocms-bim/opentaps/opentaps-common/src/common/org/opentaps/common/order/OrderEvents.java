...
public final class OrderEvents {
    ...
    public static Map prepareOrderReportParameters(DomainsLoader dl, Delegator delegator, LocalDispatcher dispatcher, GenericValue userLogin, Locale locale, String orderId, String organizationPartyId) throws GenericServiceException, GenericEntityException, PartyNotFoundException, RepositoryException, EntityNotFoundException {
        ...
        String billToPartyId = (String)order.get("billToPartyId");
        if(billToPartyId != null){
        	GenericValue person = delegator.findOne("Person", UtilMisc.toMap("partyId", billToPartyId), true);
        	if(person != null){
        		String firstNameLocal = person.getString("firstNameLocal");
        		String lastNameLocal = person.getString("lastNameLocal");
        		if(firstNameLocal != null && lastNameLocal != null){
        			order.set("billToPartyId", firstNameLocal + " " + lastNameLocal);
        		}else if(firstNameLocal == null){
        			order.set("billToPartyId", lastNameLocal);
        		}else if(lastNameLocal == null){
        			order.set("billToPartyId", firstNameLocal);
        		}
        ...
        if (client != null) {
            ...
            if (clientPhone != null) {
                ...
                if(contactMechLine.get("countryCode") == null){
                	contactMechLine.put("countryCode", "");
                }
                if(contactMechLine.get("areaCode") == null){
                	contactMechLine.put("areaCode", "");
                }
        ...
        for (OrderContactMech orderContactMech : order.getOrderContactMeches()) {
            ...
            if (contactMech.getContactMechTypeId().equals("POSTAL_ADDRESS") && contactMech.getPostalAddress() != null) {
                ...
                if(contactMechLine.get("city") != null){
                	String city = (String) contactMechLine.get("city");
                	GenericValue geo = delegator.findOne("Geo", UtilMisc.toMap("geoId", city), true);
                	contactMechLine.put("city", geo.getString("geoName"));
                }
                if(contactMechLine.get("countryGeoId") != null){
                	String countryGeoId = (String) contactMechLine.get("countryGeoId");
                	GenericValue geo = delegator.findOne("Geo", UtilMisc.toMap("geoId", countryGeoId), true);
                	contactMechLine.put("countryGeoId", geo.getString("geoName"));
                }
                if(contactMechLine.get("stateProvinceGeoId") != null){
                	String stateProvinceGeoId = (String) contactMechLine.get("stateProvinceGeoId");
                	GenericValue geo = delegator.findOne("Geo", UtilMisc.toMap("geoId", stateProvinceGeoId), true);
                	contactMechLine.put("stateProvinceGeoId", geo.getString("geoName"));
                }
                if(contactMechLine.get("countyGeoId") != null){
                	String countyGeoId = (String) contactMechLine.get("countyGeoId");
                	GenericValue geo = delegator.findOne("Geo", UtilMisc.toMap("geoId", countyGeoId), true);
                	contactMechLine.put("countyGeoId", geo.getString("geoName"));
                }
                if("000" .equals(contactMechLine.get("postalCode"))){
                	contactMechLine.put("postalCode", "");
                }
