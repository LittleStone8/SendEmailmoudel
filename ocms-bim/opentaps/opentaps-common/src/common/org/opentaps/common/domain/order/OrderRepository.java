...
public class OrderRepository extends DomainRepository implements OrderRepositoryInterface {
   ...
   private PartyRepositoryInterface partyRepository;
	...
	@Override
	public String getNeedShipment(Order order) throws RepositoryException {
	        Session session = null;
	        String isNeedShipment = null;
	        try {
	            session = getInfrastructure().getSession();
	            String sql = "select IS_NEED_SHIP from ORDER_HEADER oh where oh.ORDER_ID =?";
//	            SQLQuery createSQLQuery = session.createSQLQuery(sql);
//	            createSQLQuery. getQueryString();
	            SQLQuery query = session.createSQLQuery(sql);
	          //设置第一个参数的值为12,即查询ID=12的note
		        query.setParameter(0, order.getOrderId());
		        List list = query.list();
		        if(list.size() > 0 && list.get(0) != null){
		        	isNeedShipment = list.get(0).toString();
		        }
		        
	            return isNeedShipment;
	        } catch (InfrastructureException e) {  
	            throw new RepositoryException(e);
	        } finally {
	            if (session != null) {
	...
	@Override
	public String istoday(Order order) throws RepositoryException {
	        Session session = null;
	        try {
	            session = getInfrastructure().getSession();
	            String sql = "select oh.CREATED_STAMP from ORDER_HEADER as oh where oh.ORDER_ID=?";
//	            SQLQuery createSQLQuery = session.createSQLQuery(sql);
//	            createSQLQuery. getQueryString();
	            SQLQuery query = session.createSQLQuery(sql);
	          //设置第一个参数的值为12,即查询ID=12的note
		        ...
		        if(list.size() > 0 && list.get(0) != null){
		        	...
		        	if(time.getYear()==nowday.getYear()&&time.getMonth()==nowday.getMonth()&&time.getDay()==nowday.getDay())
		        	{
		        		return "Y";
		        	}
	...
	@Override
	public String is30day(Order order) throws RepositoryException {
	        Session session = null;
	        try {
	            session = getInfrastructure().getSession();
	            String sql = "select oh.CREATED_STAMP from ORDER_HEADER as oh where oh.ORDER_ID=?";
...
//	            createSQLQuery. getQueryString();
	          ...
	          //设置第一个参数的值为12,即查询ID=12的note
		        ...
		        if(list.size() > 0 && list.get(0) != null){
		        	Timestamp time = (Timestamp)list.get(0); 
		        	Timestamp nowday = new Timestamp(System.currentTimeMillis());
		        	long amouuth=1000L*60L*60L*24L*30L;
		        	long differencetime = nowday.getTime()-time.getTime();
		        	if(differencetime<=amouuth)
		        	{
		        		return "Y";
		        	}
		        }
	...
	@Override
	public String iscash(Order order) throws RepositoryException {
        Session session = null;
        try {
            session = getInfrastructure().getSession();
            String sql = "select opp.ORDER_ID from ORDER_PAYMENT_PREFERENCE opp where opp.PAYMENT_METHOD_TYPE_ID=\'CASH\' and opp.ORDER_ID =\'"+order.getOrderId()+"\'";
//            SQLQuery createSQLQuery = session.createSQLQuery(sql);
//            createSQLQuery. getQueryString();
            SQLQuery query = session.createSQLQuery(sql);
          //设置第一个参数的值为12,即查询ID=12的note
	...
	@Override
	public String ispacked(Order order) throws RepositoryException {
	        Session session = null;
	        try {
	            session = getInfrastructure().getSession();
	            String sql = "select oh.SHIPMENT_ID from SHIPMENT oh where oh.PRIMARY_ORDER_ID =?";
//	            SQLQuery createSQLQuery = session.createSQLQuery(sql);
//	            createSQLQuery. getQueryString();
	            SQLQuery query = session.createSQLQuery(sql);
	          //设置第一个参数的值为12,即查询ID=12的note
	...
	@Override
	public String isChargeOrder(Order order) throws RepositoryException {
	        Session session = null;
	        try {
	            session = getInfrastructure().getSession();
	            String sql = "select opp.ORDER_ID from ORDER_PAYMENT_PREFERENCE opp where opp.PAYMENT_METHOD_TYPE_ID=\'CREDIT\' and opp.ORDER_ID =\'"+order.getOrderId()+"\'";
//	            SQLQuery createSQLQuery = session.createSQLQuery(sql);
//	            createSQLQuery. getQueryString();
	            SQLQuery query = session.createSQLQuery(sql);
	          //设置第一个参数的值为12,即查询ID=12的note
	...
	@Override
	public String getOrderStroe(Order order) throws RepositoryException {
	        Session session = null;
	        try {
	            session = getInfrastructure().getSession();
	            String sql = "select ps.STORE_NAME from ORDER_HEADER as oh join  PRODUCT_STORE as ps on oh.PRODUCT_STORE_ID=ps.PRODUCT_STORE_ID where oh.ORDER_ID=?";
//	            SQLQuery createSQLQuery = session.createSQLQuery(sql);
//	            createSQLQuery. getQueryString();
	            SQLQuery query = session.createSQLQuery(sql);
	          //设置第一个参数的值为12,即查询ID=12的note
