...
public final class AutoComplete {
    ...
    private static List<GenericValue> getProductSelection(String keyword, boolean disallowVirtual, Delegator delegator) throws GenericEntityException {
        ...
        List<GenericValue> retproducts = FastList.<GenericValue>newInstance();
        if (keyword.length() > 0) {
            keyword = keyword.toUpperCase();
            ...
            TransactionUtil.begin();
            /** yzl add 2017/5/4 */
            final List<String> AC_PRODUCT_FIELDS_EX = Arrays.asList("productId", "internalName","productIdTo","description");
            EntityListIterator iterator = delegator.findListIteratorByCondition("ProductAndGoodIdentification", condition, null, AC_PRODUCT_FIELDS_EX, AC_PRODUCT_ORDER_BY, AC_FIND_OPTIONS);

            // the condition search is sufficient, so we'll just get the entire list
            products = iterator.getCompleteList();
            
            //yzl 增加如果产品变形则显示SKU,反之则显示SPU
    		//包装参数
            
			for(int i=0;i<products.size();i++)
			{
				GenericValue temp = products.get(i);
				if(temp.getString("productIdTo")==null)
				{
					temp.set("description", "");
					String internalName = temp.getString("internalName");
					if(internalName==null)
						temp.set("internalName", "");
					retproducts.add(temp);
				}
			}
			...
			String sql = "SELECT b.product_id, GROUP_CONCAT( DISTINCT DESCRIPTION ORDER BY a.PRODUCT_FEATURE_TYPE_ID ) as description FROM PRODUCT_FEATURE a LEFT JOIN PRODUCT_FEATURE_APPL b"
					+ " ON a.PRODUCT_FEATURE_ID = b.PRODUCT_FEATURE_ID where ";

			boolean isfrist = true;

			for(int j=0;j<retproducts.size();j++)
			{
				GenericValue temp = (GenericValue) retproducts.get(j);
				String productId = temp.getString("productId");
				if (isfrist) {
					sql += " product_id=\'" + productId + "\'";
					isfrist = false;
				} else {
					sql += "or product_id=\'" + productId + "\'";
				}
			}
    ...
    public static class ProductSelectionBuilder implements SelectionBuilder {
        public Map<String, Object> buildRow(Object element) {
            ...
            if(description==null)
            	description="";
            String str=product.getString("productId");
            if(product.getString("internalName")!=null&&!"".equals(product.getString("internalName")))
            {
            	str+=":"+product.getString("internalName");
            }
            if(product.getString("description")!=null&&!"".equals(product.getString("description")))
            {
            	str+=":"+product.getString("description");
            }
            return UtilMisc.<String, Object>toMap("name", str, "productId", product.getString("productId"));
        }
    }
