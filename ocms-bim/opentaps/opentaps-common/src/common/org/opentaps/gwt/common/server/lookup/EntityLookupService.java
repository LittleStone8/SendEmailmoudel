...
public abstract class EntityLookupService {
    ...
    public <T extends EntityInterface> List<T> findMyList(Class<T> entityName, EntityCondition condition, boolean paginate) {
        try {
            if (paginate) {
                boolean t = TransactionUtil.begin();
                paginateResults(getRepository().findIterator(entityName, condition, getFields(), getOrderBy()));
                ...
                boolean p = noPager;
                noPager = true;
                List<? extends EntityInterface> results = getRepository().findList(entityName, condition, getFields(), getOrderBy());
                Iterator<? extends EntityInterface> itr = results.iterator();
                EntityInterface[] orderResult = new EntityInterface[7];
                while(itr.hasNext()){
                	EntityInterface e = itr.next();
                	String statusId = e.getString("statusId");
                	/*if(!statusId.equals("ORDER_CREATED") && !statusId.equals("ORDER_COMPLETED") && !statusId.equals("ORDER_APPROVED") && !statusId.equals("ORDER_SENT") && !statusId.equals("ORDER_CANCELLED")){
                		itr.remove();
                	}*/       	        	
                	if(statusId.equals("ORDER_COMPLETED")){
                		orderResult[0] = e;
                	}
                	if(statusId.equals("ORDER_PACKED")){
                		orderResult[1] = e;
                	}
                	if(statusId.equals("ORDER_SHIPPED")){
                		orderResult[2] = e;
                	}
                	if(statusId.equals("ORDER_PAYMENT")){
                		orderResult[3] = e;
                	}
                	...
                	if(statusId.equals("ORDER_COMPLETED_W_P")){
                		orderResult[6] = e;
                	}
                }        
                results = Arrays.asList(orderResult);                
                setResults(results);
    ...
    public <T extends EntityInterface> List<T> findListNotPage(Class<T> entityName, EntityCondition condition) {
        return findMyList(entityName, condition, false);
    }
}
