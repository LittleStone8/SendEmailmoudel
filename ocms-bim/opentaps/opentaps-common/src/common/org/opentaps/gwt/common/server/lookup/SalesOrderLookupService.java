...
public class SalesOrderLookupService extends EntityLookupAndSuggestService {
    ...
    public static String findOrders(HttpServletRequest request, HttpServletResponse response) throws InfrastructureException {
        ...
        /*if(request.getRequestURI().contains("/warehouse/control")){
        	service.findOrders(Locale.US,"warehouse",1,request);
        }else {
        	if(request.getParameter("productId") == null && request.getParameter("statusId") == null && request.getParameter("productStoreId") == null){
        		service.findOrders(Locale.US,partyId,0,request);
        	}else{
        		service.findOrders(Locale.US,partyId,1,request);
        	}     	
        } */   
        service.findOrders(Locale.US,partyId,request);
     ...
     * @return the list of <code>Order</code>, or <code>null</code> if an error occurred
     */
    public List<OrderViewForListing> findOrders(Locale locale,String flag,HttpServletRequest request) {
        try {
            ...
            if (UtilValidate.isNotEmpty(getProvider().getParameter(SalesOrderLookupConfiguration.INOUT_SHIPMENT))) {
            	
                salesOrderLookupRepository.setShipment(getProvider().getParameter(SalesOrderLookupConfiguration.INOUT_SHIPMENT));
            }
            
            //*******
            if (UtilValidate.isNotEmpty(getProvider().getParameter(SalesOrderLookupConfiguration.IN_IS_FIX_ORDER))) {
            	String isFixOrder = request.getParameter("isFixOrder");
                salesOrderLookupRepository.setIsFixOrder(getProvider().getParameter(SalesOrderLookupConfiguration.IN_IS_FIX_ORDER));
            }
            
            
            
            if (UtilValidate.isNotEmpty(getProvider().getParameter(SalesOrderLookupConfiguration.INOUT_PAYMENTWAY))) {
                salesOrderLookupRepository.setPaymentWay(getProvider().getParameter(SalesOrderLookupConfiguration.INOUT_PAYMENTWAY));
            }
           
            if (UtilValidate.isNotEmpty(getProvider().getParameter(SalesOrderLookupConfiguration.INOUT_STATUS_ID))) {
            	String statusId = getProvider().getParameter(SalesOrderLookupConfiguration.INOUT_STATUS_ID);
            ...
            findAll = getProvider().getParameter(SalesOrderLookupConfiguration.IN_FIND_ALL);
            ...
            List<String> list =  getOrderBy();
            String str = list.get(0);
            if(str.contains("orderDate") && !str.contains("orderDateString")){
            	str = str.replace("orderDate", "orderDateString");
            }
            List<String> list1 = new ArrayList<String>();
            list1.add(str);
            salesOrderLookupRepository.setOrderBy(list1);
            ...
            List<OrderViewForListing> results = null;
            results = salesOrderLookupRepository.findOrders(request);
            setResults(results);
