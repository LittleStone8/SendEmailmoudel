...
public class WebAppRepository  extends Repository implements WebAppRepositoryInterface {
    ...
    public List<ShortcutGroup> getShortcutGroups(Tab tab, User user, Map<String, Object> context) throws RepositoryException {
        ...
        for (ShortcutGroup group : groups) {
            ...
            int index = 0;
            Shortcut temp = null;
            for(int k=0;k<allowedShortcuts.size();k++)
            {
            	Shortcut ww = allowedShortcuts.get(k);
            	if(ww.getLinkUrl().contains("OutgoingShipments"))
            	{
            		temp=ww;
            		flag=true;
            		index=k;
            ...
            if(flag&&index>=0&&temp!=null)
            {
            	allowedShortcuts.remove(index);
            	allowedShortcuts.add(temp);
            }
            /*yzl20170524*/
