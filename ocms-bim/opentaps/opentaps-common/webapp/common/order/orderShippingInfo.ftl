...
<#if order.shipGroups?has_content>
  ...
  <#assign sectionTitle>
    ...
    <#else>
      ...
      <@expandLabel label="CrmOrderShipGroupTo" params={"shipGroupSeqId", sg.shipGroupSeqId} />
      <#if sg.hasUnknownPostalAddress()>${uiLabelMap.CrmAddressUnknown }<#else/>${(sg.postalAddress.address1)!} - ${(sg.postalAddress.city)!}</#if>
    </#if>
    <#if order.shipGroups?size gt 1>
            ...
            <@infoRowNested title=uiLabelMap.OrderAddress>
              <div class="tabletext">
                <#if !order.isCancelled() && !order.isCompleted() && !order.isRejected()>
                  ...
                  <a id="editContactMechButton_${(shipGroup.shipGroupSeqId)!}" class="buttontext" href="#" onClick="editCurrentAddress('${shipGroup.shipGroupSeqId?if_exists}', '${shipGroup.contactMechId?if_exists}');">${uiLabelMap.CommonEdit}</a>  
                <#else/>
                <!-- yzl -->
