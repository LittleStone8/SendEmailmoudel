...
public final class InventoryServices {
    ...
    public static Map createLot(DispatchContext dctx, Map context) {
        ...
        //权限判定 chenshihua	2017-5-9
        if (!security.hasPermission("FACILITY_CREATE", userLogin)) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static Map adjustInventoryQuantity(DispatchContext dctx, Map context) throws ServiceException {
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
...
*/
        try {
            ...
            Map results = dispatcher.runSync("createPhysicalInventoryAndVariance", UtilMisc.toMap(
                 ...
                 "qoh",inventoryItem.getBigDecimal("quantityOnHandTotal").add(varianceQty),
                 "atp",inventoryItem.getBigDecimal("availableToPromiseTotal").add(varianceQty),
                 "operator",(String)userLogin.get("userLoginId")+"("+(String)userLogin.get("partyId")+")",
