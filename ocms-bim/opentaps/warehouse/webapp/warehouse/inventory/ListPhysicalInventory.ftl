...
-->
<#if security.hasEntityPermission("WRHS", "_INV_AQ", session)>
	...
	<@newPaginate name="physicalInventory" list=physicalInventory reasons=varianceReasons facilityId=facilityId productId=productId internalName=internalName >
	<#noparse >
	  ...
	  <table class="listTable">
	    <tr class="listTableHeader">
	     <#--    <@headerCell title=uiLabelMap.FormFieldTitle_inventoryItemId   orderBy="inventoryItemId" /> -->
	         <td><a href="javascript:orders('INVENTORY_ITEM_ID')">${uiLabelMap.FormFieldTitle_inventoryItemId}</a></td>
	    <#--     <@headerCell title=uiLabelMap.FormFieldTitle_productId         orderBy="productId" /> -->
	         <td><a href="javascript:orders('x.product_id')">${uiLabelMap.FormFieldTitle_productId}</a></td>  
	     <#--    <@headerCell title=uiLabelMap.ProductLotId                     orderBy="lotId" /> -->
	         <td><a href="javascript:orders('lot_id')">${uiLabelMap.ProductLotId}</a></td>
	      <#--   <@headerCell title="Product Attribute"                     orderBy="Product Attribute" /> -->
	         <td><a href="javascript:orders('internal_Name')">Model</a></td> 
	         <td><a href="javascript:orders('productAttributes')">Product Attribute</a></td>
	    <#--     <@headerCell title=uiLabelMap.FormFieldTitle_internalName      orderBy="internalName" /> -->
	       <#--  <@headerCell title=uiLabelMap.ProductItemATP                   orderBy="availableToPromiseTotal" /> -->
	        <#-- <@headerCell title=uiLabelMap.ProductItemQTY                   orderBy="quantityOnHandTotal" /> -->
	      <td><a href="javascript:orders('QUANTITY_ON_HAND_TOTAL')">${uiLabelMap.ProductItemQTY}</a></td>
	       <#--  <@headerCell title=uiLabelMap.ProductProductATP                orderBy="productATP" />-->
	       
	     <#--    <@headerCell title=uiLabelMap.ProductProductQTY                orderBy="productQOH" /> -->
	         <td><a href="javascript:orders('num')">${uiLabelMap.ProductProductQTY}</a></td>
	...
	function orders(orderBy){
		...
		location.href = "/warehouse/control/FindFacilityPhysicalInventory?facilityId="+facility_tran_002+"&orderBy="+orderBy+"&productId="+productId+"&internalName="+internalName
	}
	</script>
</#if>
