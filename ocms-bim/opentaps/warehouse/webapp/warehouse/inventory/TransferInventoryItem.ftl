...
<#else>
  ...
  <table>
	...
	<#if internalName??>
    <@displayRow title="Model" text=internalName/>
    </#if>
    <tr>
      <@displayTitleCell title=uiLabelMap.ProductSerialQty/>
          ...
          <select name="statusId" class="inputBox">
            ...
            <#list statusItems as statusItem>
              ...
              <#if ! ((! inventoryTransfer?exists) && statusItem.statusId == "IXF_COMPLETE")>
               <#if statusItem.statusId == "IXF_COMPLETE">
                   <option id="complete" value="${(statusItem.statusId)?if_exists}" ${(!(inventoryTransfer.statusId)?has_content && curStatusId == statusItem.statusId)?string("selected=\"selected\"", "")}>${(statusItem.get("description", locale))!}</option>
               <#else>
               		<#if (statusItem.statusId == "IXF_COMPLETE" || statusItem.statusId == "IXF_CANCELLED" || statusItem.statusId == "IXF_REQUESTED")>
    ...
    <tr>
     <#-- <@displayTitleCell title=uiLabelMap.ProductTransferSendDate />
      ...
      <@displayRow title=uiLabelMap.ProductTransferSendDate text=(inventoryTransfer.sendDate)! />
    </tr>
    ...
    <tr>
      <td>&nbsp;</td>
      <#if !(inventoryTransfer?exists)>
      <td class=""><input type="button" value="${uiLabelMap.ProductTransfer}" class="smallSubmit" onclick="submitFormWithSingleClick(this)"/></td> 
      <#else>
...
<span hidden id="spanForfacilityId">${inventoryTransfer.facilityIdTo}</span>
</#if>
<span id="errorMessage" hidden>${uiLabelMap.errorMessage}</span>
</@frameSection>
<script>
	this.onload = function(){
		var facilityIdTo = jQuery("#spanForfacilityId").text();
		
		if(facilityIdTo != facility_tran_002){
			jQuery("#complete").remove();
		}
