...
<#if security.hasEntityPermission("WRHS", "_INV_EDIT", session)>
...
<#--暂时隐藏转运按钮 -->
 <#-- <#assign actionLinks><a href="<@ofbizUrl>traceInventory?inventoryItemId=${inventoryItemId}&performFind=Y&traceDirection=FORWARD</@ofbizUrl>" class="subMenuButton">${uiLabelMap.WarehouseTraceInventory}</a><a href="<@ofbizUrl>TransferInventoryItem?inventoryItemId=${inventoryItemId}<#if facilityId?exists>&facilityId=${facilityId}</#if></@ofbizUrl>" class="subMenuButton">${uiLabelMap.ProductTransferItem}</a><a href="<@ofbizUrl>ViewInventoryItemDetail?inventoryItemId=${inventoryItemId}<#if facilityId?exists>&facilityId=${facilityId}</#if></@ofbizUrl>" class="subMenuButton">${uiLabelMap.ProductInventoryDetails}</a></#assign> -->
  <#assign actionLinks><a href="<@ofbizUrl>traceInventory?inventoryItemId=${inventoryItemId}&performFind=Y&traceDirection=FORWARD</@ofbizUrl>" class="subMenuButton">${uiLabelMap.WarehouseTraceInventory}</a>
 <#-- <a href="<@ofbizUrl>ViewInventoryItemDetail?inventoryItemId=${inventoryItemId}<#if facilityId?exists>&facilityId=${facilityId}</#if></@ofbizUrl>" class="subMenuButton">${uiLabelMap.ProductInventoryDetails}</a> --></#assign>
</#if>
...
<#else>
  ...
  <#else>
    <form action="<@ofbizUrl>CreateInventoryItem</@ofbizUrl>" method="post" style="margin: 0;" name="inventoryItemForm">
      ...
      <@inputHidden name="inventoryItemTypeId" value="${inventoryItemType.inventoryItemTypeId}"/>
      <@displayRow title=uiLabelMap.ProductInventoryItemTypeId text="${(inventoryItemType.inventoryItemTypeId)}" />
      <#-- <tr>
      ...
      <#if "NON_SERIAL_INV_ITEM" == (inventoryItem.inventoryItemTypeId)?if_exists>
        <@displayRow title=uiLabelMap.WarehouseQuantityQTY text="${inventoryItemData.quantityOnHandTotal?if_exists}" />
      ...
      <#elseif inventoryItem?exists>
        <tr>
          <@displayTitleCell title=uiLabelMap.ProductSerialQoh />
