...
<#if errorMessage??>
	<li id="errorMsg" class="errorMessageHeaders">${StringUtil.wrapString(errorMessage)}</li>
</#if>
<form method="post" action="<@ofbizUrl>findInventoryItem</@ofbizUrl>" name="findInventoryItem">
  ...
  <#--  <@inputLookupRow title=uiLabelMap.ProductLocation name="locationSeqId" lookup="LookupFacilityLocation" form="findInventoryItem" /> -->
    ...
    <@inputTextRow name="internalName" title="Model" />
    ...
    <#if !selectFacilityId??>
  	 <#if defaultInventoryFacilityId??><#assign defaultFacilityId = defaultInventoryFacilityId><#else><#assign defaultFacilityId = ""></#if>
  	 <#else>
  	 <#assign defaultFacilityId = selectFacilityId>
  	 </#if>
  	 <@inputSelectInventoryFacility title="Inventory Facility" name="facilityIdToFind" list=inventoryFacilitys default=defaultFacilityId />
	<@inputCheckboxRow title="QTY0" name="flag" />
