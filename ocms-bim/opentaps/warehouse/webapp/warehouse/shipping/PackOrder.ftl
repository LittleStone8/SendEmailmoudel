...
<!--                <li>
           ...
           <p><a href="<@ofbizUrl>ScheduleShipmentRouteSegment</@ofbizUrl>" class="linktext">${uiLabelMap.WarehouseScheduleThisShipment} ${shipmentId}</a></p>
         </div>
    </#if>
    <div>&nbsp;</div>
    ...
    <#if showInput != "N" && orderHeader?exists && orderHeader?has_content>
              ...
              <div class="tabletext">
                <#if postalAddress?has_content>
                  ...
                  <#else>
                    ...
                    ${postalAddress.city?if_exists}, ${postalAddress.stateProvinceGeoId?if_exists} ${postalAddress.countyGeoId?if_exists}<br>
      ...
      <#if showInput != "N" && hideGrid != "Y" && itemInfos?has_content && (totalAvailableToPack gt 0)>
        ...
        <form name="multiPackForm" method="post" action="<@ofbizUrl>ProcessBulkPackOrder</@ofbizUrl>" onsubmit="return check()" style="margin: 0;">
          <input type="hidden" name="facilityId" value="${facilityId?if_exists}">
          <input type="hidden" name="orderId" value="${orderId?if_exists}">
          <input type="hidden" name="shipGroupSeqId" value="${shipGroupSeqId?if_exists}">
          ...
          <table border='0' width="100%" cellpadding="2" cellspacing="0">
            <tr>
              ...
              <td align="right"><div class="tableheadtext">QTY</td>
            ...
            <#list itemInfos as orderItem>
              ...
              <#if inputQty gt 0>
                  <tr>
                    ...
                    <td>&nbsp;&nbsp;</td>
                    <td align="center">
                      <input type="text" class="inputBox inventoryInputBox" size="7" name="qty_${orderItem.orderItemSeqId}" value="${inputQty}">
                    </td>
                    <td align="center">
                  ...
                  <script> 
                 		 t1=${orderItemQuantity};
                 		 t2=${qohByProductId?default({})[orderItem.productId]?default(0)}
                  
                  if(t2<=0)
          		{
                	error="There is out of stock. You can call the salesman and tell him to cancel this order.";
          			issuccess= false;
          		}
          		if(t1>t2)
          		{
          			error="There is out of stock. You can call the salesman and tell him there is not enough stock.";
          			issuccess =false;
          		}
            ...
            <tr>
                ...
                <table class="tabletext" width="90%">
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="inputBox" size="15" name="packageTrackingCode_${packageSeqId}" value="${packingSession.getPackageTrackingCode_new(packageSeqId?string)?if_exists}"></td>
                    </tr>
                    </#list>
                </table>
                </#if>
...
<script type="text/javascript" src="/commondefine_js/jQuery/jquery.js"></script>
<script> 
	function check(){
		var inventoryInputBoxList = $('.inventoryInputBox');	
	var flag = true;
		if(!issuccess){
			alert(error);
			return false;
		}
		$.each(inventoryInputBoxList, function(index, elem){
				var number = $(elem).val() - 0;
